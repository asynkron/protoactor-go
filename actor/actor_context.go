...
func (ctx *actorContext) StopFuture(pid *PID) Future {
	future := newFuture(ctx.actorSystem, ctx.actorSystem.Config.StopTimeout)
...
func (ctx *actorContext) EscalateFailure(reason any, message any) {
	...
	if ctx.actorSystem.Config.DeveloperSupervisionLogging {
		ctx.Logger().Error("[Supervision] actor failed",
			slog.Any("actor", ctx.self),
			slog.Any("message", message),
			slog.Any("exception", reason),
